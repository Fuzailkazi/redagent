...
Python parity (`python/redteam.py`) is tracked as a follow-up task — not yet built.
When it is, keep it in sync with the TS implementation; do not let it rot.
...
zod · undici · vitest · OpenAI SDK (judge) · Playwright (PDF).
...
- **Keep Python at parity** once it exists. If you change scoring, detection, or
  the library contract, update `python/redteam.py` too (or open a follow-up task).
...
- **Reproducibility.** Every report pins `{ targetConfigHash, libraryVersion, engineVersion, judgeModel }`.
