...
/**
 * The engineVersion provenance pin. Phase 0 read this from the POC package.json;
 * to avoid depending on a specific package.json location the version is now
 * supplied by the caller (or via the ARMORIQ_ENGINE_VERSION env var), defaulting
 * to the same "0.1.0" the POC returned.
 */
export function getEngineVersion(version?: string): string {
  return version ?? process.env.ARMORIQ_ENGINE_VERSION ?? "0.1.0";
}
...
export interface BuildScanResultArgs {
  ...
  startedAt: string;
  finishedAt: string;
  /** Defaults to getEngineVersion() (fallback '0.1.0'). */
  engineVersion?: string;
  /** null in Phase 0 (no LLM judge). */
  judgeModel?: string | null;
  /** Pre-computed score (produced by the engine's scorer). */
  score: Score;
}
